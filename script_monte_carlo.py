...
#SBATCH --job-name=CS-monte-carlo
...
class CosmicShoreline:
    ...
    def __init__(self, data_path="/project/abbot/xuanji/2.Cosmic-Shoreline/cosmic_shoreline/data-interpolation/"):
        """
...
def age_sample_generator(num_samples,st_mass=1,V2=False,high_cutoff=None,rng= np.random.default_rng(int(time.time()))):
    if V2:
        ...
        for n in range(num_samples):
            sample = 1e8
            while sample > high_cutoff[n]:
                # Choose bin
                bin_index = rng.choice(len(bins) - 1, p=probs)
...
#     else:
#         N2O2_model = None
#     for st_mass in stellar_masses:
#         print('running for stellar mass:',st_mass)
#         # Simulate input distributions
#         stellar_mass = np.ones(num_samples) * st_mass
#         pl_insol = 10**rng.uniform(-2,5,num_samples)

#         high_cutoff = 1/stellar_mass**2.5*10

#         # if st_mass > 0.5:
#         #     stellar_age = age_sample_generator(num_samples,st_mass=1)*1e9
#         # else:
#         #     stellar_age = age_sample_generator(num_samples,st_mass=0.1)*1e9

#         stellar_age = age_sample_generator(num_samples,V2=True,high_cutoff=high_cutoff,rng=rng)*1e9

#         formation_time = 10**rng.uniform(0, 2, num_samples) * 1e6  # in years #Righter&O'brien 2011
#         xray_model = rng.choice(["Selsis", "Jackson"], num_samples)
#         euv_gamma1 = rng.uniform(-0.35 - 0.15, -0.35 + 0.07, num_samples)
#         euv_gamma2 = rng.uniform(-0.76 - 0.04, -0.76 + 0.16, num_samples)
#         CO2_fit = rng.choice(["linear", "log", "GP"], num_samples)
#         CH4_efficiency = 10 ** rng.uniform(np.log10(0.05), np.log10(0.5), num_samples)
#         N2O2_CP24_efficiency = rng.uniform(0, 1, num_samples)
...
#             pl_XUV_flux = cs._calculate_F_xuv_to_earth(L_xuv, orbital_distance)
#             pl_orbsmax[i] = orbital_distance
#             atmospheric_loss[i] = cs.integrate_carbon_loss(
#                 MMW, orbital_distance, planet_mass, stellar_mass[i],
#                 t1=formation_time[i],
...
#         if MMW == 28 and N2O2_model == "CP24":
#             df.to_csv('./data-montecarlo/df_st_mass_%.1f_MMW_%d_CP24.csv'%(st_mass, MMW))
#         else:
#             df.to_csv('./data-montecarlo/df_st_mass_%.1f_MMW_%d.csv'%(st_mass, MMW))
...
def run_func(MMW, stellar_masses, pl_masse,num_samples, proc_id):

    rng = np.random.default_rng(int(time.time()) + proc_id)

    planet_mass = np.ones(num_samples) * pl_masse
    # planet_mass = rng.uniform(pl_masse-0.5,pl_masse+0.5,num_samples)
    # Simulate input distributions
    stellar_mass = rng.uniform(stellar_masses[0],stellar_masses[1],num_samples)
    pl_insol = 10**rng.uniform(-2,5,num_samples)

    # stellar_age = rng.normal(2.8, 1, num_samples) * 1e9  # in years

    ## Swastik et al 2023 + Gadios et al 2023
    # if stellar_masses[0] > 0.5 and stellar_masses[1] > 0.5:
    #     stellar_age = age_sample_generator(num_samples,st_mass=1)*1e9
    # elif stellar_masses[0] <= 0.5 and stellar_masses[1] <= 0.5:
    #     stellar_age = age_sample_generator(num_samples,st_mass=0.1)*1e9
    # else:
    #     stellar_age_0 = age_sample_generator(num_samples,st_mass=0.1)*1e9  # in years #Swastik et al 2023
    #     stellar_age_1 = age_sample_generator(num_samples,st_mass=1)*1e9  # in years #Swastik et al 2023
    #     stellar_age = np.where(stellar_mass>0.5,stellar_age_1,stellar_age_0)
    ...
    # if MMW == 28:
    #     df.to_csv('./data-montecarlo/st_mass_0d01/df_V2_pl_mass_%.1f_st_mass_%.2f_MMW_%d_CP24.csv'%(pl_masse,stellar_masses[0], MMW))
    # else:
    #     df.to_csv('./data-montecarlo/st_mass_0d01/df_V2_pl_mass_%.1f_st_mass_%.2f_MMW_%d.csv'%(pl_masse,stellar_masses[0], MMW))
    
    # if MMW == 28:
    #     df.to_csv('./data-montecarlo/df_pl_mass_%.1f_st_mass_%.2f_MMW_%d_CP24.csv'%(pl_masse,stellar_masses[0], MMW))
    # else:
    #     df.to_csv('./data-montecarlo/df_pl_mass_%.1f_st_mass_%.2f_MMW_%d.csv'%(pl_masse,stellar_masses[0], MMW))
    if MMW == 28:
        df.to_csv('./data-montecarlo/df_pl_mass_%.1f_st_mass_%.2f_MMW_%d_CP24_3bins.csv'%(pl_masse,stellar_masses[0], MMW))
    else:
        df.to_csv('./data-montecarlo/df_pl_mass_%.1f_st_mass_%.2f_MMW_%d_3bins.csv'%(pl_masse,stellar_masses[0], MMW))
    
    return [df,high_cutoff]
...
if __name__ == '__main__':
    ...
    stellar_mass_range = [[0.1,0.2], [0.2,0.5], [0.5,1.0]]
    ...
    n_a = 0
    for MMW in MMWs:
        for stellar_masses in stellar_mass_range:
            for pl_masse in planet_masses:
                input_items.append((MMW, stellar_masses,pl_masse,num_samples,n_a))
                n_a += 1

    
    for result in p.starmap(run_func,input_items):
        print('finished %d of %d'%(n_a,len(input_items)))
        print(result[0]['st_age'])
        print(result[1])

    # Properly close the pool to ensure resources are released orderly.
    p.close()
    p.join()